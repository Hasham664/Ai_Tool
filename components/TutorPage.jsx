...
const streamRef = useRef(null);
  ...
  // ⬇️ Save to IndexedDB with max 5 entries
  const pushHistory = async (entry) => {
    const db = await getDB();
    const all = await db.getAll('history');

    if (all.length >= 3) {
      await db.delete('history', all[0].id); // delete oldest
    }
 ...
 const stopRecording = () => {
  ...
  return (
    <motion.div
      ...
      className=' dark:bg-gradient-to-b from-gray-900 via-gray-800 to-black bg-white  text-white pt-6 sm:pt-12 px-4 pb-7 flex flex-col items-center'
    >
      {/* Header */}
      <motion.div className='text-center max-w-2xl mb-8'>
      ...
      {/* Card */}
      <motion.div className='dark:bg-gray-800/60 bg-black/80 rounded-2xl p-8 shadow-lg w-full max-w-xl'>
        <div className='flex flex-col items-center space-y-6'>
          ...
          {!recording ? (
            <button
              ...
              disabled={processing}
              className='flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-primary to-chart-1 hover:from-primary/90 hover:to-chart-1/90 cursor-pointer dark:text-black text-white  font-semibold rounded-full shadow-md hover:scale-105 transition-transform duration-500 ease-in'
            >
      ...
      <div
        className='mt-6 w-full max-w-xl bg-gradient-to-r from-primary to-chart-1 dark:bg-gray-800/50 rounded-lg p-4 shadow'
      >
      ...
      {audioUrl && (
        <motion.div
          className='mt-6 bg-gradient-to-r from-primary to-chart-1 dark:bg-gray-800/50 rounded-lg p-4 shadow w-full max-w-xl'
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
