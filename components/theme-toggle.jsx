    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-[140px]">
        <DropdownMenuItem onClick={() => setTheme("light")} className="flex items-center space-x-2">
          ...
          <span>Light</span>
          {theme === "light" && <div className="ml-auto h-2 w-2 bg-primary rounded-full"></div>}
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme("dark")} className="flex items-center space-x-2">
          <Moon className="h-4 w-4" />
          <span>Dark</span>
          {theme === "dark" && <div className="ml-auto h-2 w-2 bg-primary rounded-full"></div>}
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme("system")} className="flex items-center space-x-2">
          <Monitor className="h-4 w-4" />
          <span>System</span>
          {theme === "system" && <div className="ml-auto h-2 w-2 bg-primary rounded-full"></div>}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
